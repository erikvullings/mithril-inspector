...
Ships the **source inspector**: an unobtrusive collapsed toggle,
...
- **Picker:** toggle with `Alt+Shift+M`, momentary hold with `Alt`,
...
- **Hover & highlight:** a capture-phase pointer listener uses
  ...
  frame.
- **Selection:** clicking suppresses the application click by default
...
- **Stale nodes:** a removed selection shows "Element no longer mounted"
...
- **Mapping precision:** the UI distinguishes an *exact* element source
...
- **Display names (task 0018):** the hover badge and the details
  ...
  declared one.
- **Ancestry breadcrumb & source actions (task 0019):** the
  ...
  additional location that actually resolved — component view,
...
- **Component tree (task 0022):** the sidebar's Components
  section shows the full Mithril component hierarchy — display names and
  `key="…"` badges (e.g. `UserCard key="42"`), plain HTML elements excluded by
  default — seeded once from `hook.getSnapshot()` and thereafter patched
  incrementally from batched `RuntimeEvent`s via `hook.subscribe` (no
  ...
  `role="group"`s). Selection is bidirectional: picking a DOM element (via
  ...
  every match's ancestors visible for context. Pinned components stay
  ...
  tree pane is gated by `componentTree.enabled` — the detail pane's
  ...
  prerequisite isn't met. Attrs/state render the lazy preview tree from the
  runtime's safe serializer directly as a key/value list (no redundant "Object"
  ...
  evaluated only on that explicit action), and a redacted value always
  renders its configured replacement text (default `[redacted]`) —
  the redaction itself happens in the runtime, never in this package.
- **State History (task 0027, refined task 0028):** a read-only timeline of
  ...
  no separate polling), newest first. The tab reuses the same left
...
- **Redraw-flash visualization (task 0030):** opt-in
...
- **Resilience:** every overlay operation runs inside an error boundary;
...
- **Expanding a component into its owned vnode/element tree is not
  implemented.** The tree always shows only the Mithril component hierarchy;
  there is no per-component toggle to reveal the plain DOM/vnode subtree it
  renders. Everything else (display names/keys, search, pinning, update
  counters, bidirectional selection sync, attrs/state previews, incremental
  batched updates) is implemented — this one sub-feature is deliberately
...
- **Large trees are not virtualized** (no windowing library). Collapsing a
  ...
  half of "virtualize or lazily render large trees"; true virtualization
...
- **Attrs/state pagination replaces the shown page rather than accumulating
  ...
  contract) instead of appending to what's already shown — simpler
  and still fully "paginated", but not an infinite-scroll accumulation.
- **The selected component itself does not survive a Vite full-reload** (only
  ...
  work.
- Highlight margins/padding visualization is deferred to a later release.
- Real-browser integration (playground, Chromium/Firefox/Safari) is exercised
  by the Vite playground and browser tests; this package is unit- and
...
- **A native `<dialog>` opened with `showModal()` blocks the overlay entirely.**
  ...
  now. Instead, `mountInspectorOverlay` watches for `:modal` via a
