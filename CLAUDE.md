...
- Keep `protocol`, `transform`, `runtime`, `overlay`, `server`, and `adapter-kit` independent of any specific bundler (no Vite or Rollup imports); bundler-specific wiring belongs in the adapter packages (`vite`, `rollup`, `esbuild`, ...) that consume them.
